...
internalFolder <- "manuscript" # Here to set in which folder are the scenarios I am looking for (i.e. manuscript)
...
# Here I need to randomly select areas for each and save them as new scenario names
# in Paths$outputPath, as paste0("solRas_", scen, "_Year2011.tif")
source("posthocFunctions/randomlySelectAreas.R")
...
allb <- usefulFuns::substrBoth(list.files("modules/birdsNWT/data/models/",
                                          pattern = "brt8.R"),
                               howManyCharacters = 4,
                               fromEnd = FALSE)
...
source("posthocFunctions/makeFigure3.R")
...
finalPlot <- ggplot(data = comparisonTableExtra, aes(x = Year)) +
  ...
  facet_grid(cols = vars(ProportionAreaChosen), 
             rows = vars(Habitat), scales = "free_y") +
  xlab("Time") + 
...
m3 <- lm(umbrellaIndex ~ ProportionAreaChosen + Species + ClimateScenario + Year + Run, 
         data = tableGLM)
anova(m3)
write.csv(anova(m3), file = "outputs/landscapeRuns/LandR.CS_fS/anovaLmCovariates.csv")

Require("googledrive")
drive_upload("outputs/landscapeRuns/LandR.CS_fS/anovaLmCovariates.csv", 
             as_id(figuresFolder))
  ...
  future_lapply(bibis, function(sp){ #allb # 
      ...
      lapply(scens, function(scen){
        ...
        if (redoIndividualMaps){
          mps <- makeMapsForSp(Species = sp,
                               Year = y, 
                               overwriteFig = FALSE,
                               birdSolPath = "outputs/landscapeRuns/LandR.CS_fS",
                               booSolPath = "outputs/landscapeRuns/LandR.CS_fS",
                               Scenario = scen,
                               forceUpload = FALSE,
                               outPath = "outputs/landscapeRuns/LandR.CS_fS",
    ...
    if (redoIndividualModels){
      ...
      m2 <- lm(umbrellaIndex ~ ProportionAreaChosen + Year, 
               data = subTableGLM)
      fileName <- file.path("outputs/landscapeRuns/LandR.CS_fS/",
...
lapply(c(fileNamePlot1, fileNamePlot2), 
       drive_upload, path = as_id(figuresFolder))
