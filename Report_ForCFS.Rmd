...
the proposed Edehzhie protected area. Over this region, we calculated Bird species abundance (`birdsNWT`),
                            caribou population growth (`caribouPopGrowthModel`), and caribou and relative habitat selection
                            (`caribouRSFModel`). We calculated these same indicators for two other nearby polygons of the same
                            size and shape. The purpose was to assess the performance of the proposed 
                            new protected areas with respect to these indicators.  
...
Figure 6: Caribou population growth rates. These figures show forecast demographic parameters with standard errors for the two sets of reporting regions provided `Boreal caribou NT1 range planning regions 2018` and the `Units for ECCC modeling project 2019`. Results are shown for the first four years of the simulation period only, as a sample of the preliminary results.
<center>
...
reflects only the probability of seed germination (note that re-sprouting probabilities were taken from the LANDIS-II species trait
