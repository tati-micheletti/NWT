...
potential in these respects of specific candidate protected areas. The integrated model is implemented as a suite of eight modules in
...
that are not forested from their analyses; their outputs are based on forested land classes (see the `LandR` module description, and
...
largest and most comprehensive integration of landscape and climate change effects on Species At Risk to date. It represents a novel and
...
statistical relationships documented at regional and national scales. These processes may not accurately predict localized processes -
this discrepancy between scales is well documented in ecology (Wiens 1989; Holling 1992; Levin 1992; Burton et al. in review) with
...
\textcolor{blue}{scfm-fireSense}: `scfm-fireSense` is a fire hybrid of two structurally similar models, `scfm` and `fireSense`. In each
                                  ...
                                  occurs. The arrival (or ignition) process is modeled by stating fires within cells according to some
                                  low probability. 
...
increase in spread probability. Lastly, a function minimizer, coupled with scam predict method, is used to find the spread probability
...
fire ignition model. fireSense depends on a fuel type a classifier built within the XGBoost library (Chen and Guestrin 2016) that
...
The full version of the fireSense model has spatially and temporally variable spread parameters that vary with fuel type and annual fire
...
scfm-fireSense is less sensitive to variation in vegetation and to changes in fire weather than the complete fireSense model would be.


## MARIO STILL TO PROVIDE INFORMATION FOR THIS ##
\textcolor{blue}{anthropogenic}: To be completed by MT.
IMPORTANT ASSUMPTION TO ADD: This module does not model seismic lines - see caveat for `caribouPopGrowthModel`
...
\textcolor{blue}{caribou}: The `caribou` module is composed of two main modules:  `caribouRSFModel` and `caribouPopGrowthModel`. Each
                           module loads existing glms developed by Environment Canada 2011 to project caribou occurrence and
...
4. Shrub and Herbaceous landcover remains constant across the simulation period (2010 to 2110) despite the impacts of fire, succession,
...
6. Caribou do not display season-specific habitat selection; the RSF model used here is based off of annual caribou occurrences. It does
