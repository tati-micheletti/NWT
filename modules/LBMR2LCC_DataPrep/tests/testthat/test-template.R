...
#    `functionName` is a function's name in your module (e.g., `LBMR2LCC_DataPrepEvent1`).
# 2. Copy this file to the tests folder (i.e., `LBMR2LCC_DataPrep/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("LBMR2LCC_DataPrep")
  path <- list(modulePath = "..",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    LBMR2LCC_DataPrep = list(.saveInitialTime = NA)
  )
  ...
  if (exists("LBMR2LCC_DataPrepEvent1", envir = .GlobalEnv)) {
    simOutput <- LBMR2LCC_DataPrepEvent1(mySim)
  } else {
  ...
  if (exists("LBMR2LCC_DataPrepEvent2", envir = .GlobalEnv)) {
    simOutput <- LBMR2LCC_DataPrepEvent2(mySim)
  } else {
