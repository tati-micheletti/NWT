...
test_that("test Event1 and Event2.", {
  ...
  parameters <- list(
    ...
    correlateRasters = list(.saveInitialTime = NA)
  )
  ...
  if (exists("correlateRastersEvent1", envir = .GlobalEnv)) {
    simOutput <- correlateRastersEvent1(mySim)
  } else {
  ...
  if (exists("correlateRastersEvent2", envir = .GlobalEnv)) {
    simOutput <- correlateRastersEvent2(mySim)
  } else {
