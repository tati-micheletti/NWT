...
#    `functionName` is a function's name in your module (e.g., `fireSense_NWT_DataPrepEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/jemar270/Documents/GitHub/fireSense_NWT_DataPrep/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fireSense_NWT_DataPrep")
  path <- list(modulePath = "C:/Users/jemar270/Documents/GitHub",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    fireSense_NWT_DataPrep = list(.saveInitialTime = NA)
  )
  ...
  if (exists("fireSense_NWT_DataPrepEvent1", envir = .GlobalEnv)) {
    simOutput <- fireSense_NWT_DataPrepEvent1(mySim)
  } else {
  ...
  if (exists("fireSense_NWT_DataPrepEvent2", envir = .GlobalEnv)) {
    simOutput <- fireSense_NWT_DataPrepEvent2(mySim)
  } else {
