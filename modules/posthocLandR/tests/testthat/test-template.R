...
#    `functionName` is a function's name in your module (e.g., `posthocLandREvent1`).
# 2. Copy this file to the tests folder (i.e., `/mnt/data/Micheletti/NWT/modules/posthocLandR/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("posthocLandR")
  path <- list(modulePath = "~/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    posthocLandR = list(.saveInitialTime = NA)
  )
  ...
  if (exists("posthocLandREvent1", envir = .GlobalEnv)) {
    simOutput <- posthocLandREvent1(mySim)
  } else {
  ...
  if (exists("posthocLandREvent2", envir = .GlobalEnv)) {
    simOutput <- posthocLandREvent2(mySim)
  } else {
