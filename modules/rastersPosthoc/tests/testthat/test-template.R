...
#    `functionName` is a function's name in your module (e.g., `rastersPosthocEvent1`).
# 2. Copy this file to the tests folder (i.e., `/mnt/data/Micheletti/NWT/modules/rastersPosthoc/tests/testthat`).
...
test_that("test Event1 and Event2.", {
  ...
  parameters <- list(
    ...
    rastersPosthoc = list(.saveInitialTime = NA)
  )
  ...
  if (exists("rastersPosthocEvent1", envir = .GlobalEnv)) {
    simOutput <- rastersPosthocEvent1(mySim)
  } else {
  ...
  if (exists("rastersPosthocEvent2", envir = .GlobalEnv)) {
    simOutput <- rastersPosthocEvent2(mySim)
  } else {
