...
#    `functionName` is a function's name in your module (e.g., `waterlandClassificationEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/tmichele/Documents/GitHub/NWT/modules/waterlandClassification/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("waterlandClassification")
  path <- list(modulePath = "/home/tmichele/Documents/GitHub/NWT/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    waterlandClassification = list(.saveInitialTime = NA)
  )
  ...
  if (exists("waterlandClassificationEvent1", envir = .GlobalEnv)) {
    simOutput <- waterlandClassificationEvent1(mySim)
  } else {
  ...
  if (exists("waterlandClassificationEvent2", envir = .GlobalEnv)) {
    simOutput <- waterlandClassificationEvent2(mySim)
  } else {
