...
outputsLandR <- data.frame(
  objectName = rep(c("burnMap",
                 ...
                 "simulationOutput",
                 "pixelGroupMap",
                 "simulatedBiomassMap"), each = succTS),
...
assign(paste0("LandR_SCFM", runVersion), value = simInitAndSpades(times = times, params = parameters,
...
# assign(paste0("birdsFireCaribou", runVersion), value = simInitAndSpades(times = times, params = parameters,
#                     modules = modulesBirdsFireCaribou,
#                     objects = .objects, paths = paths,
#                     loadOrder = unlist(modulesBirdsFireCaribou),
#                     outputs = outputsLandR, debug = 2))
...
saveRDS(object = get(paste0("LandR_SCFM", runVersion)), 
        file = file.path(paths$outpu, paste0("birdsFireCaribou", runVersion,"_", toupper(format(Sys.time(), "%d%b%y")))))
}
```
...
df <- comparePredictions(polyList = names(birdsFireCaribouV2$scfmDriverPars), simList = birdsFireCaribouV2) %>%
...
### RESULTS V1
It is important to note that the version 1 of the integrated model contains only the following modules:
LandR_Biomass: forest succession
scfm: simple fire model
caribouPopGrowth: simple population growth model
birdsNWT: birds model

Disadvantages of this version:
1. It completely lacks climate sensitivity (vegetation dynamics and in the fire model)
2. scfm *might* be underestimating fires
3. Caribou is showing population instead of lambda
4. Caribou pop growth lacks K, environmental stochasticity, has only one model (M3)
5. Reporting for Caribou population growth is for 
6. No caribouRSF
