...
\textcolor{blue}{LandR}: This group of modules is composed of one module and one group of modules.                          The first module, `Boreal_LBMRDataPrep` creates the specific parameters 
...
\textcolor{blue}{birdsNWT}: The birds module loads an existing glm object for each sp located in  
                            ...
                            module) will be added to the nect version of these models.
...
\textcolor{blue}{caribou}: The `caribou` module is composed of two main modules: `caribouRSF` and `caribouPopGrowth`. 
                           ...
                           lack of data for the specific location, and initial population size was set to `200`. 
...
whichComputer <- "BorealCloud"

paths <- pathsSetup(whichComputer = whichComputer) 
if (length(paths$inputPath) == 1) paths$modulePath <- c(paths$modulePath, file.path(paths$modulePath, "scfm/modules"))

if (pemisc::user() %in% c("Tati", "tmichele")) {
    setTempFolder(paths = paths, setTmpFolder = TRUE, usr = pemisc::user())
}
...
Edehzhie.url <- "https://drive.google.com/open?id=0B_2riEic8l1mcXJWblRoSllMNGM4N2ZWa0JadGNaWDlJR3RB"
...
modulesBirds_Fire <- list("scfmLandcoverInit", "scfmRegime", "scfmDriver", "scfmIgnition", "scfmEscape", "scfmSpread", "Boreal_LBMRDataPrep", "LandR_BiomassGMOrig","Biomass_regeneration", "LBMR", "birdsNWT") # Still need to finish
# modulesBirdsFireCaribou <- list("scfmLandcoverInit", "scfmRegime", "scfmDriver", "scfmIgnition", "scfmEscape", "scfmSpread", "Boreal_LBMRDataPrep", "LandR_BiomassGMOrig","Biomass_regeneration", "LBMR", "birdsNWT", "caribouRecDistRelationship", "caribouPopGrowthModel") # Still need to implement the disturbance
...
outputsLandR <- data.frame(
  objectName = c("cohortData",
                 ...
                 "summaryBySpecies1",
                 "simulationOutput"),
  file = c(paste0("cohortData_", toupper(format(Sys.time(), "%d%b%y"))),
...
# SCFM <- simInitAndSpades(times = times, params = parameters,
#                     modules = modulesSCFM,
#                     objects = .objects, paths = paths,
...
# wetlands <- simInitAndSpades(times = times, params = parameters, 
#                     modules = modulesWetland, 
#                     objects = .objects, paths = paths, 
#                     loadOrder = unlist(modulesWetland), debug = 2)
birdsAndFire <- simInitAndSpades(times = times, params = parameters,
                    modules = modulesBirds_Fire,
                    objects = .objects, paths = paths,
                    loadOrder = unlist(modulesBirds_Fire), debug = 2)
# birdsFireCaribou <- simInitAndSpades(times = times, params = parameters,
#                     modules = modulesBirdsFireCaribou,
#                     objects = .objects, paths = paths,
#                     loadOrder = unlist(modulesBirdsFireCaribou), debug = 2)
