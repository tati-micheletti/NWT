...
system(paste0("find . -size +50M | cat >> .gitignore"), wait = TRUE)
...
message(paste0("This model uses ", sessionInfo()[["R.version"]][["version.string"]]))

```

Second, please make sure you have all needed dependencies. If you don't have all 
packages installed yet, please first update all your packages and install SpaDES. 
...
```{r initSubmodules}
system("git submodule init", wait = TRUE)

# Set origin for all submodules
system(paste0("cd ", getwd(), 
              " && cd modules/LBMR ",
              " && git remote set-url origin git@github.com:eliotmcintire/LBMR.git"), wait = TRUE)
system(paste0("cd ", getwd(), 
              " && cd modules/Boreal_LBMRDataPrep ",
              " && git remote set-url origin git@github.com:eliotmcintire/Boreal_LBMRDataPrep.git"), wait = TRUE)
system(paste0("cd ", getwd(), 
              " && cd modules/LandR_BiomassGMOrig ",
              " && git remote set-url origin git@github.com:eliotmcintire/LandR_BiomassGMOrig.git"), wait = TRUE)
system(paste0("cd ", getwd(), 
              " && cd modules/Biomass_regeneration ",
              " && git remote set-url origin git@github.com:eliotmcintire/Biomass_regeneration.git"), wait = TRUE)
system(paste0("cd ", getwd(), 
              " && cd NationalModel ",
              " && git remote set-url origin git@github.com:dstralberg/NationalModel.git"), wait = TRUE)
system(paste0("cd ", getwd(), 
              " && git submodule foreach git fetch"), wait = TRUE) # make sure you have development branch as an option")
system(paste0("cd ", getwd(), 
              " && git submodule foreach git pull"), wait = TRUE) # make sure you have development branch as an option")
system(paste0("cd ", getwd(), 
              " && git pull"), wait = TRUE)
```
...
# print(sppEquivalencies_CA[,.(LandR, EN_generic_full, Latin_full)], 
#                 nrow = nrow(sppEquivalencies_CA))
...
SpaDES.core::objectSynonyms(envir = mySim@.envir, synonyms = list(c("studyArea", "studyAreaLarge"))) # Why do I have to pass this?
