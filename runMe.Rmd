...
whichComputer <- if (pemisc::user("emcintir")) "LocalMachine" else if (pemisc::user("tmichele")) "BorealCloud" else "LocalMachine"
...
In order to have a map of the first fires, we call SCFM lasting the recovery time Caribou
needs from the forest to support the population

```{r spinnup}
# Spinnup the fire to generate the last 40 years of burns in the area (for the Caribou model)

parametersSpinnup <- parameters
parametersSpinnup$scfmSpread$spinnup <- TRUE
# Not cloud cached because of backfile Rasters
              # Changing these times may have umpredictable consequences for caribou population growth internally. 
              # Use with caution! [ FIX ] Add this as stop in scfmSpread Init()
SCFM <- Cache(simInitAndSpades, times = list(start = 0, end = parameters$caribouPopGrowthModel$recoveryTime+1), 
              params = parametersSpinnup,
              modules = modulesSCFM,
              objects = .objects, paths = paths,
              loadOrder = unlist(modulesSCFM), debug = 2,
              omitArgs = c("destinationPath", "debug"))

.objects[["cumulBurn"]] <- SCFM$cumulBurn
```
