...
library("LandR")
invisible(system(paste0("find . -size +50M | cat >> .gitignore | 
                        sort .gitignore | uniq "), wait = TRUE))
...
# system("git submodule sync | git submodule update --init --recursive --remote", wait = TRUE)
...
.objects <- list(
  "studyArea" = studyArea,
  "studyAreaLarge" = studyArea, # Passing to try to see if I can fix the recursive problem without having to give it too much thought...
