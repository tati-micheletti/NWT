...
birdsFireCaribouV2 <- simInitAndSpades(times = times, params = parameters,
                    modules = modulesBirdsFireCaribou,
                    objects = .objects, paths = paths,
                    loadOrder = unlist(modulesBirdsFireCaribou),
                    outputs = outputsLandR, debug = 2)
...
# To save a run
saveRDS(object = birdsFireCaribouV2, file = file.path(getwd(), paste0("outputs/birdsFireCaribouV2_", toupper(format(Sys.time(), "%d%b%y")))))
```


### Extracting some extra plots
```{r extra_plots}
