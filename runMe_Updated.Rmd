...
<!-- JEAN TO Check INFORMATION on FIRE SENSE-->
...
based on biomass but on age. Pixels are considered disturbed if the time since last fisturbance is less than 16 years. This module
relies on a classifier build within the XGBoost library (Chen and Guestrin 2016) this this translation. Table 2 summarises the accuracy
...
`fireSense_FrequencyFit` is an implimentation of the method described in Marchal et al. (2017); it fits a statistical model used to
...
Code (MDC) of July and introducted vegetation sensitivity using five vegetation classes. In this project, we use the MDC of June along
...
for the non "non-fuel" class as it will not burn - it therefore remains as 0. This module outputs a fitted model object of class
...
probability surface is updated annually, used to feed the escape component of `fireSense`.
...
`fireSense` then passes the information on the pixels that are burning to `scfmSpread`, which spreads the fires.
