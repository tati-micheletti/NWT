...
\textcolor{blue}{fireSense}: `fireSense` is a wildfire simulation module that is seneitive to both climate and vegetation change; it reproduces the spatial and temporal variation of both the number of fires, and fire escape probability. `fireSense` is composed of two groups of modules; a group that prepares and formats the data (`climate_NWT_DataPrep`, `fireSense_NWT_DataPrep`, `LBMR2LCC_DataPrep`, and `MDC_NWT_DataPrep`), and a group that impliments the algorithms needed to simulate fire (`fireSense_FrequencyFit`, `fireSense_FrequencyPredict`, `fireSense_EscapeFit`, and `fireSense_EscapePredict`).





This group of modules is composed of four submodules. `fireSense_FrequencyFit` fits a
