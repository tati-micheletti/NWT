...
<!--FRANCES TO UPDATE WITH CERES FINAL COMMENTS - ONGOING -->
\textcolor{blue}{LandR}: `LandR` is a landscape dynamic vegetation model; it simulates landcape-scale forest dynamics in a spatio
                          termporally explicit manner. The `LandR` group of modules is composed of one data treatment and
                          parameterisation module, and one group of modules that simulate biomass usccession dynamics. 
...
these site specific parameters are Bird Conservation Region (BCR) 6 within the Northwest Territories. For instance, maximum biomass,
above-ground net primary productiviey (aNPP), and establishment probabilities from seed germination are calculated separately depending
on tree species, ecodistrict, land cover, stand age, and species cover. This module also provides other parameters, such as species
tolerances to shade, and plant traits (e.g. longevity, ability to resprout, etc.). These traits were obtained from trait tables used in
LANDIS-II, a popular forest-landscape simulation model, and are available from Dominic Cyr's GitHub page: 
...
LANDIS-II Biomass Success module (v6.2), but have since been changed (i.e. spin-up was removed; Barros et al. in prep). They consist of
(1) `LBMR`, the `LandR` module responsible for vegetation aging, dispersal, updating biomass folowing other modules' events, and
producing summary figures and tables; (2) `LandR_BiomassGMOrig`, the `LandR` module responsible for cohort growth and mortality; and
(3) `Biomass_regeneration`, the `LandR` module which handles post-disturbance biomass regneration (e.g. fire). `LandR` needs at least
the first two modules (`LBMR` and `LandR_BiomassGMOrig`) to produce sensible vegetation dynamics.

In brief, the `LandR` modules reproduce forest biomass dynamics in a spatially explicit manner at the landscape scale. They simulate
...
automatically calibrate themselves. We have identified some limitations arising from this caveate:
...
longevity are known to be flawed. On the one the one hand, estimating stand age can be tricky and often relies on allometry
...
2. Because the `LandR` model is suited for forest dynamics, pixeLs that correspond to non-forested LCC05 classes were not used to
...
biomass conditions. However, Biomasses will still need to be adjusted to match the observed current biomass, and this requires starting
with even-aged stands in places where forest may have a complex age structure and mixed species composition. An alternate solution may
be to let the model run for a few hundred timesteps (*i.e.* a stabilization phase) until vegetation dynamics reach quasi-equilibrium,
before introducing the subsequent modules within this project that rely on results from `LandR` (*i.e.* `birdsNWT`, `caribouRSFModel`,
and `caribouPopGrowthModel`). This change would address current `LandR` limitation #1.

2. Models such as `LandR` will often overestimate tree cover and biomass because they lack quantified ecological mechanisms that limit
tree growth (*i.e.* differences in growing season length, soil nutrient availability, or water availability). Future versions on the
`LandR` modules could ignore this limitation, or incorporate estimates for these mechanisms, allowing  `LandR` to predict for ares that
do not coresspond to purely forested areas. This change would address current `LandR` limitation #2.


## JEAN TO ADD INFORMATION TO FIRE SENSE, ALEX TO ADD INFORMATION TO FEENIX (SCFM)##
\textcolor{blue}{fireSense}: `fireSense` is a wildfire simulation module that is seneitive to both climate and vegetation change; it
...
groups of modules; a group that prepares and formats the data (`climate_NWT_DataPrep`, `fireSense_NWT_DataPrep`, `LBMR2LCC_DataPrep`,
and `MDC_NWT_DataPrep`), and a group that impliments the algorithms needed to simulate fire (`fireSense_FrequencyFit`,
...
`LBMR2LCC_DataPrep` translates `LandR_Biomass` outputs into landcover classes as needed by `fireSense` modules. It relies on a
classifier built with the XGBoost library (Chen and Guestrin 2016) for the translation. The following table summarises the classifier
accuracy per landcover class on the validation dataset.

![Table 1. Classifer accuracy per landcover class on the validation dataset]
|Landcover class            | Accuracy|
|:-------------------------:|:--------|
...
`fireSense_FrequencyFit` is an implimentation of the method described in Marchal et al. (2017); it fits a statistical model used to parameterize the fire ignition component of `fireSense`. Marchal et al. (2017) introduced climate sensitivity using the Monthly Drought Code (MDC) of July and introducted vegetation sensitivity using five vegetation classes. This this project, we use the MDC of June along with nine vegetation classes, all of which are derived from the 39 classes of the Land Cover Map of Canada 2005 (LCC05) (Table 2). We did not estimate the fire frequency for the non
