...
important contribution to ecological forecasting, and contains the ability to uniquely address both national (i.e. *Species At Risk
                                                                                                              Act*) and international (i.e. *Convention on Biological Diversity Aichi Target 11*) policy mandates for biodiversity conservation today,
...
statistical relationships documented at regional and national scales. These processes may not acurately predict localized processes -
  this discrepency between scales is well documented in ecology (Wiens 1989; Holling 1992; Levin 1992) with direct ramifications on
downscaling models (e.g. Riiters 2005; Araujo et al. 2005). Future iterations of this project may be able to address these
...
targetFile = "waterRasterNW.tif", studyArea = studyAreaNWT, 
...
```
  ...
  ### Climate sensitive fire
  
  Now we try running LandR with climate sensitivity for fire (fireSense+SCFM)
```{r LandR_fS_Carib}
  
  # /!\ Added cache id to modelBiomass, fireSense. Needed for saving time!!! remove after run!
...
```{r caribPop_Landr_fS_Edehzhie}
  ...
  # 
  # runVersion <- "V1"
  # assign(paste0("LandR_FireSense_CaribouPopGrw_", runVersion),
  #        value = simInitAndSpades(inputs = inputs, times = times,
  #                                 params = parameters,
  #                                 modules = modulesLandR_fS_SCFM_CaribPop,
  #                                 objects = .objects, paths = paths,
  #                                 loadOrder = unlist(modulesLandR_fS_SCFM_CaribPop),
  #                                 outputs = outputsLandR, debug = 2))
  
```
  
  LandRCS + fireSense-SCFM + Caribou Pop Growth
```{r allNWT}
  # modulesLandRCS_fS_SCFM_CaribouPop <- c("Boreal_LBMRDataPrep", 
  #                             "LandR_BiomassGMOrig",
  #                             "Biomass_regeneration", 
  #                             "LBMR",
  #                             "climate_NWT_DataPrep", 
  ...
  # .objects <- c(.objects, list("studyAreaPSP" = studyAreaPSP))
  # 
  # runVersion <- "V1"
  # assign(paste0("LandRCSFireSenseCaribPop_", runVersion), 
  #        value = simInitAndSpades(inputs = inputs, times = times, 
  #                                 params = parameters,
  #                                 modules = modulesLandR_fS_FEENIX,
  #                                 objects = .objects, paths = paths,
  #                                 loadOrder = unlist(modulesLandR_fS_FEENIX),
  #                                 outputs = outputsLandR, debug = 2)) # FAILING
```
