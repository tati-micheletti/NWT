...
The `caribou` group of modules has two main components:  `caribouRSFModel` and `caribouPopGrowthModel`. Each
                           module applies statistical models developed by Environment Canada (2011) to project caribou occurrence and
                           demographics over 100 years. The statistical models respond to the amount and location of 
                           ...
                           our study area of BCR6 in the NWT, which corresponds closely to the Taiga Plain ecozone.       

`caribouRSFModel`
The statistical relationships between caribou locations detectect by telemetry and habitat characteristics have been 
...
According to the selected RSF model, relative probabilities of habitat selection by adult, female, boreal caribou are best described by
...
The methods behind model development can be found in Environment Canada (2011) Appendix 7.3. 
Note that the RSF model predicts relative selection probability, rather than absolute probability, 
as a use-available design was used (Manly et al. 2002)

Model covariates were either static or dynamic. Static covariates were determined by initial conditions. Dynamic covariates were
determined by raster layers maintained by one or other of the ecological process modules. 
Covariate types and sources are given in the following table: 
...
The RSF model is applied on a yearly (or decadal?) time step. The output is a raster layer of resouce selection probabilities across the study area. A measure of model prediction error is also produced.
<!-- more details needed -->.
...
2. Resource selection probability is positivey correlated with population fitness and population density (Manly et al. 2002). .
...
4. Shrub and Herbacious landcover remains constant across the simulation period (2010 to 2110) independent of fire, succession,
