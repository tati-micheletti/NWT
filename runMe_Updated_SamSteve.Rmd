...
that are not forested from their analyses; their outputs are based on forested landclasses (see the `LandR` module description, and
...
\textcolor{blue}{LandR}: `LandR` is a landscape dynamic vegetation model; it simulates landscape-scale forest dynamics in a spatio
                          ...
                          parameterisation module, and a group of modules that simulate biomass succession dynamics. These modules
                          estimate parameters for the study area specified. The methods and results of these estimations can be found
                          in Appendix 1.
...
competition for light resources, seed dispersal and germination, regeneration following a disturbance, and background or fire-related
mortality.
...
<!--MARIO STILL TO REVIEW THIS-->
\textcolor{blue}{anthropogenic}: The `anthropogenic` module aims to project future anthropogenic disturbances, calculate road density
                                 as km/km2 within a 10km radius, and compute a 500 meter buffer around all anthropogenic disturbances.
                                 It is composed of three submodules: `anthrDisturbanceNWT`, `lineDensity`, and `Buffer`. It provides
                                 information to the `birdsNWT`, `caribouPopGrowth`, and `caribouRSFModel` modules.
...
anthrDisturb. This layer is updated by adding disturbance features as indicated in a feature update schedule file 
anthrDisturbSchedule.txt, which provides a planning schedule and contains columns with information on scheduling and feature
descriptives (i.e. Class, name, shortName, fileName and url) of planned disturbances. Currently, only the proposed extention of the
McKenzie highway is added. Other planned disturbances can easily be added to the simulation by including them to
anthrDisturbSchedule.txt available in the module’s data directory. 

The `anthrDisturbanceNWT` module uses the Boreal ecosystem anthropogenic disturbance (BEADS) vector data from two sources for seven
anthropogenic features. BEADS data for Canada (2008 to 2010) is available from 
(https://open.canada.ca/data/en/dataset/afd0ce47-17c3-445c-b823-2f86409da2e0). The 2010 road network file for the NWT is available from
...
3  powerlines (BEADS)
4  railways (BEADS)
5  roads (NWT road network file)
6  seismic lines (BEADS)
7  unknown (BEADS)

The `lineDensity` module computes densities of linear feature within a specific radius around each raster cell, expressed as magnitude
per-unit area (km/km2). The default input object is `anthrDisturb`, from which features can be selected. In this model only the roads
layer of anthrDistur is used to calculate road density within a 10 km radius around each raster cell. The road density output
LFDensityMap is masked to waterRaster, removing road density values in cells that represent water.

`lineDensity` currently uses an approach that approximates line densities. First, lengths of linear features within each raster cell
are calculated. The value for line length within each cell is then spread out over all neighbouring cells within a 10km radius and
summed where they overlap using fast fourier transforms (fft). This is equivalent to summing total line length within a 10 km circle
around each cell divided by circle area. These calculations can be done on smaller ‘tiles’ of the study area to reduce computational
requirements and computation time (but has not been parallelized yet). The magnitude-per-unit area (km/km2) is calculated by dividing
these summed values by pixel area (which makes m/m2) and multiplied by a factor 1000 to convert them to km/km2. These road density
estimates are used in the `caribouRSFModel` module.

The `Buffer` module computes a buffer layer (bufferMap) of buffers with a specific radius around features that are being selected from
the anthrDisturb object. The buffer radius can be varied, and the buffer layer can be converted to a raster object if desired. The
default raster identifies all cells that are covered by the buffer layer, and assignings them a value 1; cells that are not covered by
the buffer layer are assigned a value of 0 to creat a binomial rater layer. These raster computations can be done for all features
together or by feature Class. This output, BufferMap, is masked to waterRaster, removing road density values in cell that represent
water. This raster is used in the `caribouPopGrowth` module.

*Important assumptions of the `anthropogenic` module*
1. All linear features are created within one time step, regardless of feature length.

2. Anthropogenic features do no deteriorate, recover, or become restored throughout the simulated period.

3. Fit is an appropriate method for the calculated of feature densities (specific to the `lineDensity` module).

4. Only spatial line objects are present on the landscape (specific to the `Buffer` module)
...
2. Deterioiration or recovery of anthropogenic features through time (to address assumption #2)
...
link to a future module simulating timber harvest that keeps track of cut-block location, age, and yeild curves. A review of
preliminary modules written by Cumming for use in SpaDES teaching found that they were not yet ready ready to be adapted to this
project. The development work behind a forest harvesting module are planned for the summer.

4. Interaction between `LandR` and this `anthropogenic` module sot hat pixels that intersect with disturbance features can be
classified as disturbed in vegetation succession models.

5. Call python code to emulate ArcMap's lineDensity function (sensu Environment Canada 2011, Appendix 7.3).

6. Incorportion of spatial objects other than spatial lines (to address assumption #4).

7. An output could be provided to calculate the percentage of a cell withing the BufferMap that that is covered by the buffer layer of
each feature class.

8. The ability to vary the buffer sizes by feature class, which would allow testing of the effect of buffer sizes on caribou
...
Cumming, S. G., D. Demarchi and C. Walters. 1998. A Grid-Based Spatial Model of Forest Dynamics Applied to the Boreal Mixedwood
Region. Working Paper 1998–8. Sustainable Forest Management Network. https://doi.org/10.7939/R35N33.

Armstrong, G. W. and S. G. Cumming. 2003. Estimating the cost of land base changes due to wildfire using shadow prices. Forest Science,
49(5):719-730.
...
Marchal, J., S. G. Cumming, and E. J. B McIntire. 2017. Land Cover, More than Monthly Fire Weather, Drives Fire-Size Distribution in
Southern Québec Forests: Implications for Fire Risk Management. PLoS ONE 12(6):1–17. https://doi.org/10.1371/journal.pone.0179294.
...
Pya, N. S. N. Wood. 2015. Shape constrained additive models. Statistics and Computing, 25(3):543-559.
