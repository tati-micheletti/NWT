...
The `lineDensity` module calculates road density (km/km^2) within a 10km search radius (sensu Environment Canada 2011), and uses
anthrDisturb from the `anthrDisturbanceNWT` module as an input. The `lineDensity` module provides an input to the `caribouRSFModel`
module.

The `Buffer` module computes a 500 m buffer around all anthropogenic disturbances, and uses antrDisturb from the `anthrDisturbanceNWT`
module as an input. The `Buffer` module provides an input to the `caribouPopGrowth` module.
...
2. Deterioiration or recovery of anthropogenic features through time (to addess assumption #2)
...
4. Interaction between `LandR` and this `anthropogenic` module sot hat pixels that intersect with disturbance features can be classified as disturbed in vegetation succession models.

To be completed by MT.
