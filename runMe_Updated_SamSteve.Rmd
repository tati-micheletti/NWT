...
The `caribou` group of modules has two main components:  `caribouRSFModel` and `caribouPopGrowthModel`. Each
                           ...
                           The specific statistical model selected for the `caribouRSFModel` module was approporiate to 
                           our study area of BCR6 in the NWT, which corresponds closely to the Taiga Plains ecozone. The specific
                           statistial model selected for the `caribouPopGrowthModel` module was appropriate for all boreal caribou
                           across Canada.      
...
for various parts of Canada. We used the top RSF-type model, a binomial GLMM, for the Taiga Plains ecozone (Environment Canada 2011; Table 46). This model was estimated from individual carobou location data collected from 24 Alberta, 50 British Columbia,
and 169 NWT VHF-collared adult female caribou between 2000 and 2010 (Environment Canada 2011; Table 39). The analysis used a habitat use-availabilty design, so the model predicts relative, not absolute, resource selection (Manly et al. 2002).
...
The RSF model is applied on a yearly time step. The output is a raster layer of relative resource selection across the study area. A map of model prediction error (standard error) is also produced.
<!-- more details needed -->
...
2. Resource selection is positivley correlated with population fitness and population density (Manly et al. 2002). .
...
The `caribouPopGrowthModel` is based on the statistical relationship between the landscape and a single demographic parameter, recruitment, that underpins Canada's national woodland boreal caribou recovery strategy (Environment Canada 2011; 2012). The critical demographic paramater is the annual rate of calf recruitment ($R$), measured as caribou calves per 100 adult female at the time of winter flight surveys. The data used here are estimated for "caribou study areas", in effect the various spatial units over which  aunual caribou surveys were conducted (Environment Canada 2011). 
The best statistical model of these data (Environment Canada 2011; Table 55, Model M3) was determined as:
<!-- is it Table 55 or 56? -- FS: It is table 55-->
...
where "total disturbance" over each study area is “the percent total non-overlapping fire and anthropogenic disturbances
 (Environment Canada 2011; Table 54). `Anthropogenic disturbance` is mapped from the union of 500m buffers applied to all anthropgenic disturbances (see the `andthropogenic` module). `Fire disturbance` is the proportional area burned within the past 40 years (see the `scfm-fireSense`. For methodological details, see Appendix 7.6 of Environment Canada (2011) (Appendix 7.6).
 
$R$ is predicted over each polygon within the two coverages shown in [Figure 2] and [Figure 3]. The total disturbance covariate is calculated for each polygon from raster laters maintained by the `scfm-fireSense` and `anthropogenic` modules. These rasters are overlaid and summed to calculate the percentage non-overlapping area of total disturbance within each polygon. This prediction is an annual calculation.

The demographic parameter reported for each polygon is the estimated annual per-capita population growth rate ($\lambda$). 
Using a well published two-stage population structure of adult females and juveniles (Hatter and Bergerud 1991; McLoughlin et al. 2003; Sorensen et al. 2006; Hervieux et al. 2014), we can calculate:

$$\lambda = (1 - M)/(1 - R),$$  [Eqn. 3]

where $R$ is juvenile female recruitment as predicted from [Eq. 2], and $M = 1-S$ where $S$ is the adult female survivorship. 
We assume fixed $S=0.85$, as was done in Environment Canada's recovery strategy reports (Environment Canada 2011, 2012). We report $\lambda$ periodically for the regions in [Figure 2](Boreal_caribou_NT1_range_planning_regions_2018.jpg), and [Figure 3](Units_for_ECCC_modeling_project_2019.jpg). We note that any desired reporting units, or periods, could be accomodated given the appropriate shapefile. 
...
4. Movement rates of indivduals between reporting polygons are negligible, i.e. these polygoins are functionally isolated
from one another.

*Future versions of the `caribouPopGrowthModel` module could include:*  

1. Revised model paramaters or demographic values appropriate to the spatial locations of desired predictions.
...
linking cell-level habitat quality with demographic paramaters (see Environment Canada 2011, Table 55, model M12).
...
3. Species turnover based on community dissimilarities under alternate landuse or climate scenarios (sensu Stralberg et al. 2009)

\pagebreak                           

### **Edehzhie**
For the third deliverable, we have conducted a limited multi-species analysis over 
the proposed Edehzhie potected area. Over this region, we calculated Bird species abundance (`birdsNWT`),
                            caribou population growth (`caribouPopGrowthModel`), and caribou and relative habitat selection
                            (`caribouRSFModel`). We caclulated these same indicators for two other nearby polygons of the same
                            size and shape. The purpose was to assess the performance of the proposed 
                            new potected areas with respect to these indicators.  
...
Figure 5:  American Redstart (AMRE) and Overnbird (OVEN) abundance prediction without climate sensitive vegetation (`LandR`) or fire (`scfm`).
<!-- ![Figure 5](outputs/28MAR19/AMREpredNWT_30MAR19.gif) -->
<center>
![Figure 5](outputs/28MAR19/BirdPredictionExample.png)
</center>
Figure 7: Caribou selection preference over the three areas.
<center>
...
Figure 6: Caribou population growth rates. These figures show forecast demographic paramaters with standard errors for the two sets of reporting regions provided `Boreal caribou NT1 range planning regions 2018` and the `Units for ECCC modeling project 2019`. Results are shown for the first four years of the simulation period only, as a sample of the preliminary results.
<center>
![Figure 6a](outputs/28MAR19/Area1.png)
![Figure 6b](outputs/28MAR19/Area2.png)
</center>

Figure 8a and b: Community metrics for the three areas, showing 4 diversity indices in 2011 and 2111.

<center>
![Figure 8a](outputs/28MAR19/DiversityIndexYear0.png)  
![Figure 8b](outputs/28MAR19/DiversityIndexYear100.png)
</center>

\pagebreak

Figure 9: Community metrics for the three areas showing 4 diversity indices for each polygon for every 10 years of simulations.

<center>

![Figure 9](outputs/28MAR19/CommunityMetrics.png)
...
carbou within proyected areas and eslewhere, to evaluate potential for multi-species conservation planning. 
It was not possible to complete this analysis by deadline. We will pursue it in April.
...
This project represents a first comprehensive effort in data integration and ecological modelling to demonstrate a multi-species approach to manageing populations and habitats of Species At Risk in the Nothwest Territories, if not in all of Canada. This project project illusstrates the potential of large initiatives involving multiple Departments of government and academic researchers collaborating to solve complex problems at the nexus of science and policy. The project also demonstrates the potential of open source modelling tools cand reproducible work flow. 
...
verifiably and reproducibly linked to the best avalable data, than can account for the future state of our ecosystems under 
...
elsewhere in Canada.  
 ...
 <!-- 
Within this project we have provided the required bird
...
$$B ~ \log(\mathrm{Age} * \mathrm{speciesCode} + \mathrm{cover} * \mathrm{speciesCode} + (\mathrm{speciesCode} | \mathrm{ecoregionGroup})$$ [Eq. 2]
...
as $1 - (1 - \mathrm{predictedCover})^10$. These estimates were then multiplied by 
$(1 - \mathrm{resprouting probability})$ of each species, so that SEP
