...
The purpose of this project was to support multi-species modelling objectives of Environment and Climate Change Canada with respect to
...
The integrated model is used to forecast the spatial distribution of selected songbird species and the regional potential to sustain viable populations of woodland
...
in all modules for processing and reporting. Most reporting is done over forested habitats only, neglecting waterbodies, wetlands, and areas of tundra vegetation or
...
At Risk to date. It represents a novel and important contribution to ecological forecasting, and contains the ability to uniquely
address both national (i.e. *Species At Risk Act*) and international (i.e. *Convention on Biological Diversity Aichi Target 11*) policy
...
specific conditions at any given location and time. This discrepency between regional and local scales is well documented in ecology
...
<!-- HERE DOWN STILL NEEDS STEVE'S EDITS-->
### **birdsNWT**  
This module projects species abundance of bird species under forecasted landscape and climate
...
BRTs (see list below). For this project, predictions were performed for 10 species, including:
...
** Current state: ** This module is completely functional, but depends on the model objects being accessed and loaded in R for the predictions. The fully reproducible workflow (i.e. statistical model + predictions) might be incorporated in the future.


### **caribou**  
The `caribou` group of modules is composed of two main ones:  `caribouRSFModel` and `caribouPopGrowthModel`. Each
                           ...
                           natural features (`LandR` module) of the NWT BCR6 region (i.e. Taiga plains) change. Combined, these
...
years of the simulation, as well as an uncertainty map of the relative selection (i.e. standard deviation of the mean
relative selection at each pixel).
...
7. OldBurns are considered to be anything older than 40 years. We assume that OldBurns are above 41 (Environment Canada 2011, Appendix 7.3, section 5.3;1917 to 2010). 


Future versions of the `caribouRSFModel` module could;
...
than the Taiga Plains RSF modeled here. That depends on the preparations of several extra layers for prediction.
...
on  a yearly time interval, and uses these data to determine the population growth rate, lambda ($\lambda$).
...
removed)" (Environment Canada 2011, Table 54). `Fire disturbance` in the proportion of the landscape burned in the last < 40 years within a caribou study area. `Anthropogenic disturbance` is measured as the proportion of non-overlapping anthropogenic disturbance with a 500 m buffer within a caribou study area
(Environment Canada 2011; Table 52). This module uses the rasterized output maps of the `fire` module, and
`anthropogenic` modules. These map the presence/absence of fire < 40 years old, or anthropogenic disturbances, at each 250 m x 250m
...
study area for each time step (i.e. annual), and are used as inputs for this statistical relationship (Eq 2.).
...
population size (lambda; $\lambda$) - for each polygon in [Figure 2] and [Figure 3]. The population model used to estimate lambda on a yearly
...
(M) is calculated as 1 - adult female survival (SadF). At each yearly time step lambda is saved. This module outputs a plot of lambda, for
each requested region or caribou planning area [Figure 2](Boreal_caribou_NT1_range_planning_regions_2018.jpg), [Figure 3](Units_for_ECCC_modeling_project_2019.jpg), over the simulated time interval (100 years; 2010 through 2110).
...
Future versions of the `caribouPopGrowthModel` module could address many of the above assumptions. An updated module could propose;
1. Revised caribou study areas (or local population units - the recommended scale of population management; Environment
Canada 2012), demographic values, and/or statistical models to incorporate contemporary data under updated data sharing
agreements.

2a. Flexibility to test multiple top statistical models presented in Environment Canada 2011 (i.e. rather than solely Eq 3.; Table 55,
Table 56) or future reports.

2b. The output of the `caribouRSFModel` as a predictor in the `caribouPopGrowthModel` statistical model. This would explicitly
link these two modules by using high quality habitat as a predictor of caribou recruitment  (sensu Environment Canada 2011,
Table 55, model M12).

3. A statistical relationship between recruitment and landscape change that is specific to the NWT (addressing assumption #
1 above).

4. A statistical relationship between adult female survival and landscape change that is specific to the NWT (addressing
assumption # 2 above).

5. Incorporation of a caribou population model that is both logistic (i.e. density dependent) and/ or stochastic to look at
projected variations in population size.

** Current state: ** Both these modules are functional, and currently depend only on having outputs from LandR and a fire model. There is, however, the need of an ecological sanity check on the results these are producing. 
...
###**comm_metricsNWT**   
This module computes a series of avian community metrics based on the suite of species-level
...
3. Species turnover based on community dissimilarity that considers different climate change scenarios (sensu Stralberg et al. 2009)
