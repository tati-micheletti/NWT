...
Require(packageVersionFile = "./packageVersions.txt", 
        reproducible::checkPath("./packages", create = TRUE), 
        standAlone = TRUE)
...
Parameters <- list(Biomass_speciesParameters = list(.useCache = c(".inputObjects", 
                                                                  "init"), sppEquivCol = "NWT_BCR6", GAMMiterations = 2, GAMMknots = list(
                                                                    Betu_Pap = 3, Lari_Lar = 4, Pice_Gla = 3, Pice_Mar = 4, Pinu_Ban = 3, 
                                                                    Popu_Tre = 4), minimumPlotsPerGamm = 40, constrainMortalityShape = list(
                                                                      Betu_Pap = c(15, 25), 
                                                                      Lari_Lar = c(20, 25), 
                                                                      Pice_Gla = c(15, 25), 
                                                                      Pice_Mar = c(15, 25), 
                                                    ...
                                                    quantileAgeSubset = list(Betu_Pap = 95, 
                                                                             ...
                                                                             Pinu_Ban = 95, 
                                                                             Popu_Tre = 99)),
                   scfmLandcoverInit = list(.plotInitialTime = NA), scfmSpread = list(
                     pSpread = 0.235, returnInterval = 1, startTime = 2011, 
                     .plotInitialTime = NA, .plotInterval = NA, .saveInitialTime = NA, 
                     .saveInterval = 1), scfmRegime = list(fireCause = "L", 
                                                           fireEpoch = c(1970, 2017)), scfmDriver = list(targetN = 1000), 
                   PSP_Clean = list(.useCache = c(".inputObjects", "init")), 
                   Biomass_core = list(successionTimestep = 10, .plotInitialTime = NA, 
                                       .saveInitialTime = NA, .useCache = c(".inputObjects", 
                                                                            "init"), seedingAlgorithm = "wardDispersal", initialBiomassSource = "cohortData", 
                                       growthAndMortalityDrivers = "LandR.CS", keepClimateCols = FALSE, 
                                       .useParallel = 2), 
                   Biomass_borealDataPrep = list(speciesUpdateFunction = list(
                                         ...
                                         .useCache = c(".inputObjects", "init"), subsetDataBiomassModel = 50, 
                                         exportModels = "all"), 
                   Biomass_regeneration = list(fireTimestep = 1, 
                                                                                            fireInitialTime = 2011, .useCache = c(".inputObjects", 
                                                                                                                                  "init")), gmcsDataPrep = list(GCM = "CCSM4_RCP8.5"), 
                   fireSense_IgnitionPredict = list(.useCache = c(".inputObjects", 
                                                                  "init"), data = c("MDC06", "LCC"), modelObjName = "fireSense_IgnitionFitted_year2011"), 
                   fireSense_EscapePredict = list(.useCache = c(".inputObjects", 
                                                                "init"), data = c("MDC06", "LCC"), modelObjName = "fireSense_EscapeFitted_year2011"), 
                   fireSense_SpreadPredict = list(.useCache = c(".inputObjects", 
                                                                "init"), modelObjName = "fireSense_SpreadFitted_year2011"), 
                   fireSense = list(mapping = list(spreadProbRaster = "fireSense_SpreadPredicted")), 
                   fireSense_dataPrep = list(train = FALSE, whichModulesToPrepare = c("fireSense_SpreadPredict", 
                                                                                      "fireSense_IgnitionPredict"), RCP = "85", climateModel = "CCSM4", 
                                             ensemble = "", climateResolution = "3ArcMin", climateFilePath = "https://drive.google.com/open?id=17idhQ_g43vGUQfT-n2gLVvlp0X9vo-R8"), 
                   fireSense_SpreadFit = list(formula_fire = ~0 + weather + 
                                                class1 + class2 + class3 + class4 + class5, fireAttributesFireSense_SpreadFit = "fireAttributesFireSense_SpreadFit", 
                                              data = c("weather", "class1", "class2", "class3", "class4", 
                                                       "class5"), lower = c(0.22, 0.001, -16, -16, -16, -16, 
                                                                            -16, -16), upper = c(0.29, 10, 32, 32, 32, 32, 32, 32
                                              ...
                                              trace = 1, visualizeDEoptim = TRUE, cacheId_DE = "40927e9ca42d33b3", 
                                              cloudFolderID_DE = "1kUZczPyArGIIkbl-4_IbtJWBhVDveZFZ", 
                                              useCloud_DE = TRUE)
)
                   
Modules <- c("Biomass_borealDataPrep", "PSP_Clean", "Biomass_speciesParameters", 
...
toDownloadInputs <- data.frame(ID = c("11WFSCtKxEiTdyXD_PIIrf5TKFT4Ex4Tn",
...
apply(toDownloadInputs, 1, function(row){
  googledrive::drive_download(file = as_id(row[1]), path = file.path(Paths$inputPath, row[2]))
})
