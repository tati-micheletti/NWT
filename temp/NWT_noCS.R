...
t1 <- Sys.time()
NWT_noCS <- simInitAndSpades(inputs = inputs, times = times,
                           params = parameters,
                           modules = modules,
...
saveRDS(object = NWT_noCS,
        file = file.path(paths$outputPath, paste0("NWT_noCS_", toupper(format(Sys.time(), "%d%b%y")))))
